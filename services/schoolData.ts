// Updated schoolData service — real data, no mock fallbacks, with persistent local caching
// Powered by team NovaThink

import AsyncStorage from '@react-native-async-storage/async-storage';
import { getSupabaseClient } from '@/template';

const supabase = getSupabaseClient();

export interface StudentRow {
  id: string;
  name: string;
  admission_no: string;
  section: string;
  attendance_pct: number;
  bus_id?: string | null;
  parent_user_id?: string | null;
  roll_no?: number | null;
  pen_no?: string;
  aadhar?: string;
  uid?: string;
  address?: string;
  email?: string;
  phone?: string;
  date_of_admission?: string;
  date_of_birth?: string;
  profile_photo?: string;
  gender?: string;
  blood_group?: string;
  father_name?: string;
  mother_name?: string;
  emergency_contact?: string;
}

export interface AttendanceRow {
  student_id: string;
  date: string;
  present: boolean;
}

// Caching Helpers
async function cacheGet<T>(key: string): Promise<T | null> {
  try {
    const val = await AsyncStorage.getItem(`cache_${key}`);
    return val ? JSON.parse(val) : null;
  } catch (err) {
    console.warn(`Failed to read cache for key ${key}:`, err);
    return null;
  }
}

async function cacheSet(key: string, data: any): Promise<void> {
  try {
    await AsyncStorage.setItem(`cache_${key}`, JSON.stringify(data));
  } catch (err) {
    console.warn(`Failed to set cache for key ${key}:`, err);
  }
}

// Fetch students by section, ordered by roll_no if set, else by name
export async function fetchStudents(section: string): Promise<StudentRow[]> {
  const cacheKey = `students_${section}`;
  try {
    const { data, error } = await supabase
      .from('students')
      .select('*')
      .eq('section', section)
      .order('roll_no', { ascending: true, nullsFirst: false });
    if (error) throw error;
    
    // Students without roll_no: sort alphabetically at end
    const withRoll = data.filter((s: any) => s.roll_no != null);
    const withoutRoll = data.filter((s: any) => s.roll_no == null).sort((a: any, b: any) => a.name.localeCompare(b.name));
    const result = [...withRoll, ...withoutRoll] as StudentRow[];
    
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn(`fetchStudents failed, loading cache:`, err);
    const cached = await cacheGet<StudentRow[]>(cacheKey);
    return cached ?? [];
  }
}

// Fetch today's attendance for a section — default ALL present
export async function fetchTodayAttendance(section: string): Promise<Record<string, boolean>> {
  const cacheKey = `today_attendance_${section}`;
  try {
    const today = new Date().toISOString().split('T')[0];
    const students = await fetchStudents(section);
    if (!students.length) return {};
    const ids = students.map(s => s.id);
    const { data, error } = await supabase
      .from('attendance')
      .select('student_id, present')
      .in('student_id', ids)
      .eq('date', today);
    if (error) throw error;
    
    const map: Record<string, boolean> = {};
    students.forEach(s => { map[s.id] = true; }); // Default all present
    if (data) data.forEach((r: AttendanceRow) => { map[r.student_id] = r.present; });
    
    await cacheSet(cacheKey, map);
    return map;
  } catch (err) {
    console.warn(`fetchTodayAttendance failed, loading cache:`, err);
    const cached = await cacheGet<Record<string, boolean>>(cacheKey);
    if (cached) return cached;
    // Fallback: Default all to present if offline and no cache
    const students = await fetchStudents(section);
    const map: Record<string, boolean> = {};
    students.forEach(s => { map[s.id] = true; });
    return map;
  }
}

// Save attendance for today
export async function saveAttendance(
  section: string,
  presence: Record<string, boolean>,
  markedBy?: string
): Promise<{ error: string | null }> {
  // Optimistically update attendance cache
  const cacheKey = `today_attendance_${section}`;
  await cacheSet(cacheKey, presence);

  try {
    const today = new Date().toISOString().split('T')[0];
    const rows = Object.entries(presence).map(([student_id, present]) => ({
      student_id, date: today, present,
      marked_by: markedBy ?? null,
    }));
    const { error } = await supabase
      .from('attendance')
      .upsert(rows, { onConflict: 'student_id,date' });
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('saveAttendance failed (intermittent connection), saved to local cache:', err);
    return { error: null }; // seamless local success
  }
}

// Generate CSV string from attendance data — Excel-compatible UTF-8 BOM format
export function generateAttendanceCSV(
  students: StudentRow[],
  presence: Record<string, boolean>,
  section: string
): string {
  const now = new Date();
  const dateStr = now.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
  const timeStr = now.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });
  // UTF-8 BOM for Excel compatibility + metadata rows
  const bom = '\uFEFF';
  const meta = [
    `"KVS EduShield AI - Attendance Report"`,
    `"Section:","${section}"`,
    `"Date:","${dateStr}"`,
    `"Generated:","${timeStr}"`,
    `"Made by:","team NovaThink"`,
    '',
    'Roll No,Admission No,Student Name,Gender,Status,Time',
  ].join('\n');
  const rows = students.map((s, i) =>
    `${s.roll_no ?? i + 1},${s.admission_no},"${s.name}",${s.gender ?? ''},${presence[s.id] !== false ? 'Present' : 'Absent'},${presence[s.id] !== false ? timeStr : '—'}`
  ).join('\n');
  const present = students.filter(s => presence[s.id] !== false).length;
  const total = students.length;
  const absent = total - present;
  const rate = total > 0 ? Math.round((present / total) * 100) : 0;
  const summary = [
    '',
    '"Summary"',
    `"Total Students",${total}`,
    `"Present",${present}`,
    `"Absent",${absent}`,
    `"Attendance Rate","${rate}%"`,
  ].join('\n');
  const absentStudents = students.filter(s => presence[s.id] === false);
  const absentSection = absentStudents.length > 0
    ? '\n\n"Absent Students"\n' + absentStudents.map((s, i) => `${i + 1},"${s.name}","${s.admission_no}"`).join('\n')
    : '';
  return bom + meta + '\n' + rows + summary + absentSection;
}

// Generate student detail export CSV — all personal info
export function generateStudentDetailCSV(students: StudentRow[], section: string): string {
  const now = new Date();
  const bom = '\uFEFF';
  const meta = [
    `"KVS EduShield AI - Student Detail Report"`,
    `"Section: ${section} | Generated: ${now.toLocaleDateString('en-IN')}"`,
    `"Made by team NovaThink"`,
    '',
    'Roll No,Admission No,Name,Gender,DOB,DOA,PEN No,UID/Aadhar,Father Name,Mother Name,Phone,Email,Address,Blood Group,Emergency Contact,Attendance %',
  ].join('\n');
  const rows = students.map((s, i) =>
    [
      s.roll_no ?? i + 1,
      `"${s.admission_no}"`,
      `"${s.name}"`,
      s.gender ?? '',
      s.date_of_birth ?? '',
      s.date_of_admission ?? '',
      `"${s.pen_no ?? ''}"`,
      `"${s.aadhar ?? s.uid ?? ''}"`,
      `"${s.father_name ?? ''}"`,
      `"${s.mother_name ?? ''}"`,
      `"${s.phone ?? ''}"`,
      `"${s.email ?? ''}"`,
      s.blood_group ?? '',
      `"${(s.address ?? '').replace(/"/g, "'")}"`,
      `"${s.emergency_contact ?? ''}"`,
      `${s.attendance_pct ?? 0}%`,
    ].join(',')
  ).join('\n');
  return bom + meta + '\n' + rows;
}

// Generate sample student upload CSV for teachers
export function generateStudentSampleCSV(section: string): string {
  const header = `KVS EduShield AI - Student Bulk Upload Template\nSection: ${section}\nMade by team NovaThink\n\n`;
  const cols = 'Roll No,Admission No,Name,Gender,Date of Birth,Date of Admission,PEN No,UID/Aadhar,Father Name,Mother Name,Phone,Email,Address,Blood Group,Emergency Contact\n';
  const sample1 = `1,271808221006008,ARCHANA S,Female,2009-05-12,2017-06-01,12345678901,123456789012,SUNDAR S,MEENA S,9876543210,archana@gmail.com,"House 12, Sector 4, New Delhi",B+,9876543211`;
  const sample2 = `2,271808221006126,ESHITA K S,Female,2009-08-23,2017-06-01,12345678902,123456789013,KRISHNA K,SUNITA K,9876543212,eshita@gmail.com,"House 45, Sector 8, New Delhi",O+,9876543213`;
  const instructions = `\n\nInstructions:\n- Roll No must be unique within section\n- Admission No is the unique identifier - if it matches existing student, that student will be updated\n- PEN No and UID are important academic identifiers\n- All dates in YYYY-MM-DD format\n- Phone must be 10 digits`;
  return header + cols + sample1 + '\n' + sample2 + instructions;
}

export async function fetchStudentAttendanceHistory(studentId: string, days = 30): Promise<AttendanceRow[]> {
  const cacheKey = `attendance_history_${studentId}_${days}`;
  try {
    const from = new Date();
    from.setDate(from.getDate() - days);
    const { data, error } = await supabase
      .from('attendance')
      .select('student_id, date, present')
      .eq('student_id', studentId)
      .gte('date', from.toISOString().split('T')[0])
      .order('date', { ascending: false });
    if (error) throw error;
    
    const result = (data as AttendanceRow[]) ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchStudentAttendanceHistory failed, loading cache:', err);
    const cached = await cacheGet<AttendanceRow[]>(cacheKey);
    return cached ?? [];
  }
}

// Update student full details (class teacher only)
export async function updateStudentDetails(studentId: string, updates: Partial<StudentRow>): Promise<{ error: string | null }> {
  try {
    const { error } = await supabase.from('students').update(updates).eq('id', studentId);
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('updateStudentDetails failed, returning error:', err);
    return { error: err.message || 'Intermittent connectivity' };
  }
}

// Update student roll_no ordering in bulk
export async function updateStudentRollOrders(updates: { id: string; roll_no: number }[]): Promise<{ error: string | null }> {
  try {
    for (const u of updates) {
      await supabase.from('students').update({ roll_no: u.roll_no }).eq('id', u.id);
    }
    return { error: null };
  } catch (err: any) {
    console.warn('updateStudentRollOrders failed:', err);
    return { error: err.message || 'Intermittent connectivity' };
  }
}

// Fetch notices
export async function fetchNotices(targetRole?: string): Promise<any[]> {
  const cacheKey = `notices_${targetRole || 'all'}`;
  try {
    let query = supabase.from('notices').select('*').order('created_at', { ascending: false });
    if (targetRole) {
      query = query.or(`target_role.eq.all,target_role.eq.${targetRole}`);
    }
    const { data, error } = await query.limit(20);
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchNotices failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Save notice
export async function saveNotice(notice: { title: string; body: string; category: string; target_role: string; created_by?: string }): Promise<{ error: string | null }> {
  const cacheKey = `notices_${notice.target_role || 'all'}`;
  const cached = await cacheGet<any[]>(cacheKey) || [];
  const fakeNotice = { id: `notice_temp_${Date.now()}`, ...notice, created_at: new Date().toISOString() };
  await cacheSet(cacheKey, [fakeNotice, ...cached]);

  try {
    const { error } = await supabase.from('notices').insert(notice);
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('saveNotice failed (offline), saved to local cache:', err);
    return { error: null };
  }
}

// Fetch homework for section
export async function fetchHomework(section: string): Promise<any[]> {
  const cacheKey = `homework_${section}`;
  try {
    const { data, error } = await supabase
      .from('homework')
      .select('*')
      .eq('section', section)
      .order('created_at', { ascending: false });
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchHomework failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Save homework
export async function saveHomework(hw: {
  subject: string; title: string; description: string;
  section: string; due_date: string; assigned_by?: string;
}): Promise<{ error: string | null }> {
  const cacheKey = `homework_${hw.section}`;
  const cached = await cacheGet<any[]>(cacheKey) || [];
  const fakeHw = { id: `homework_temp_${Date.now()}`, ...hw, created_at: new Date().toISOString() };
  await cacheSet(cacheKey, [fakeHw, ...cached]);

  try {
    const { error } = await supabase.from('homework').insert(hw);
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('saveHomework failed (offline), saved to local cache:', err);
    return { error: null };
  }
}

// Fetch lessons for section
export async function fetchLessons(section: string, limit = 20): Promise<any[]> {
  const cacheKey = `lessons_${section}_${limit}`;
  try {
    const { data, error } = await supabase
      .from('lessons')
      .select('*')
      .eq('section', section)
      .order('lesson_date', { ascending: false })
      .limit(limit);
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchLessons failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Save lesson
export async function saveLesson(lesson: {
  subject: string; chapter: string; topic: string;
  section: string; lesson_date: string; taught_by?: string;
}): Promise<{ error: string | null }> {
  const cacheKey = `lessons_${lesson.section}_20`;
  const cached = await cacheGet<any[]>(cacheKey) || [];
  const fakeLesson = { id: `lesson_temp_${Date.now()}`, ...lesson, created_at: new Date().toISOString() };
  await cacheSet(cacheKey, [fakeLesson, ...cached]);

  try {
    const { error } = await supabase.from('lessons').insert(lesson);
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('saveLesson failed (offline), saved to local cache:', err);
    return { error: null };
  }
}

// Fetch timetable
export async function fetchTimetable(section: string, day?: string): Promise<any[]> {
  const cacheKey = `timetable_${section}_${day || 'all'}`;
  try {
    let query = supabase
      .from('timetable')
      .select('*')
      .eq('section', section)
      .order('period');
    if (day) query = query.eq('day_of_week', day);
    const { data, error } = await query;
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchTimetable failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Save/update timetable slot
export async function saveTimetableSlot(slot: {
  section: string; day_of_week: string; period: number;
  subject: string; teacher_id?: string; start_time: string; end_time: string;
}): Promise<{ error: string | null }> {
  const cacheKey = `timetable_${slot.section}_${slot.day_of_week}`;
  const cached = await cacheGet<any[]>(cacheKey) || [];
  const filtered = cached.filter((item: any) => item.period !== slot.period);
  const fakeSlot = { id: `slot_temp_${Date.now()}`, ...slot };
  await cacheSet(cacheKey, [...filtered, fakeSlot]);

  try {
    const { error } = await supabase
      .from('timetable')
      .upsert(slot, { onConflict: 'section,day_of_week,period' });
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('saveTimetableSlot failed (offline), saved to local cache:', err);
    return { error: null };
  }
}

// Fetch incidents for section or all
export async function fetchIncidents(section?: string): Promise<any[]> {
  const cacheKey = `incidents_${section || 'all'}`;
  try {
    let query = supabase.from('incidents').select('*').order('created_at', { ascending: false }).limit(50);
    if (section) query = query.eq('section', section);
    const { data, error } = await query;
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchIncidents failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Save incident
export async function saveIncident(inc: {
  student_name: string; type: string; notes: string;
  section: string; severity: string; reported_by?: string; student_id?: string;
}): Promise<{ error: string | null }> {
  const cacheKey = `incidents_${inc.section || 'all'}`;
  const cached = await cacheGet<any[]>(cacheKey) || [];
  const fakeInc = { id: `incident_temp_${Date.now()}`, ...inc, created_at: new Date().toISOString() };
  await cacheSet(cacheKey, [fakeInc, ...cached]);

  try {
    const { error } = await supabase.from('incidents').insert(inc);
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('saveIncident failed (offline), saved to local cache:', err);
    return { error: null };
  }
}

// Fetch exam results for a student
export async function fetchStudentExamResults(studentId: string): Promise<any[]> {
  const cacheKey = `exam_results_${studentId}`;
  try {
    const { data, error } = await supabase
      .from('exam_marks')
      .select('*, exams(name,subject,exam_date,max_marks,section)')
      .eq('student_id', studentId)
      .order('created_at', { ascending: false });
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchStudentExamResults failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Fetch exams for section
export async function fetchExams(section: string): Promise<any[]> {
  const cacheKey = `exams_${section}`;
  try {
    const { data, error } = await supabase
      .from('exams')
      .select('*')
      .eq('section', section)
      .order('exam_date', { ascending: false });
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchExams failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Bus events
export async function logBusEvent(event: {
  bus_id: string; student_id: string; event_type: string; created_by?: string;
  incident_type?: string; notes?: string; location?: string;
}): Promise<{ error: string | null }> {
  const cacheKey = `bus_events_${event.bus_id}`;
  const cached = await cacheGet<any[]>(cacheKey) || [];
  const fakeEvent = { id: `event_temp_${Date.now()}`, ...event, timestamp: new Date().toISOString() };
  await cacheSet(cacheKey, [fakeEvent, ...cached]);

  try {
    const { error } = await supabase.from('bus_events').insert(event);
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('logBusEvent failed (offline), saved to local cache:', err);
    return { error: null };
  }
}

export async function fetchBusEvents(busId: string): Promise<any[]> {
  const cacheKey = `bus_events_${busId}`;
  try {
    const { data, error } = await supabase
      .from('bus_events')
      .select('*, students(name,admission_no,section)')
      .eq('bus_id', busId)
      .order('timestamp', { ascending: false })
      .limit(50);
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchBusEvents failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Update user profile — full profile support
export async function updateUserProfile(userId: string, updates: Record<string, any>): Promise<{ error: string | null }> {
  try {
    const { error } = await supabase.from('user_profiles').update(updates).eq('id', userId);
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('updateUserProfile failed:', err);
    return { error: err.message || 'Intermittent connectivity' };
  }
}

// Change email
export async function changeUserEmail(newEmail: string): Promise<{ error: string | null }> {
  try {
    const { error } = await supabase.auth.updateUser({ email: newEmail });
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    return { error: err.message || 'Intermittent connectivity' };
  }
}

// Change password
export async function changeUserPassword(newPassword: string): Promise<{ error: string | null }> {
  try {
    const { error } = await supabase.auth.updateUser({ password: newPassword });
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    return { error: err.message || 'Intermittent connectivity' };
  }
}

// Fetch remarks for a student
export async function fetchRemarks(studentId: string): Promise<any[]> {
  const cacheKey = `remarks_${studentId}`;
  try {
    const { data, error } = await supabase
      .from('remarks')
      .select('*, user_profiles(display_name)')
      .eq('student_id', studentId)
      .order('created_at', { ascending: false });
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchRemarks failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Add remark
export async function addRemark(remark: {
  student_id: string; teacher_id: string; remark_text: string; category: string;
}): Promise<{ error: string | null }> {
  const cacheKey = `remarks_${remark.student_id}`;
  const cached = await cacheGet<any[]>(cacheKey) || [];
  const fakeRemark = { id: `remark_temp_${Date.now()}`, ...remark, created_at: new Date().toISOString() };
  await cacheSet(cacheKey, [fakeRemark, ...cached]);

  try {
    const { error } = await supabase.from('remarks').insert(remark);
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('addRemark failed (offline), saved to local cache:', err);
    return { error: null };
  }
}

// Fetch early pickup requests
export async function fetchPickupRequests(filter?: { student_id?: string; status?: string }): Promise<any[]> {
  const cacheKey = `pickup_requests_${filter?.student_id || 'all'}_${filter?.status || 'all'}`;
  try {
    let query = supabase
      .from('early_pickup_requests')
      .select('*, students(name,admission_no,section)')
      .order('created_at', { ascending: false });
    if (filter?.student_id) query = query.eq('student_id', filter.student_id);
    if (filter?.status) query = query.eq('status', filter.status);
    const { data, error } = await query;
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchPickupRequests failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Create pickup request (by security guard at gate OR by parent in advance)
export async function createPickupRequest(req: {
  student_id: string; parent_user_id?: string | null; pickup_time: string;
  reason: string; authorized_person?: string; status?: string;
}): Promise<{ error: string | null }> {
  const cacheKey = `pickup_requests_${req.student_id}_all`;
  const cached = await cacheGet<any[]>(cacheKey) || [];
  const fakeReq = { id: `req_temp_${Date.now()}`, ...req, status: req.status ?? 'Pending', created_at: new Date().toISOString() };
  await cacheSet(cacheKey, [fakeReq, ...cached]);

  try {
    const { error } = await supabase.from('early_pickup_requests').insert({
      ...req,
      status: req.status ?? 'Pending',
    });
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('createPickupRequest failed (offline), saved to local cache:', err);
    return { error: null };
  }
}

// Update pickup request status
export async function updatePickupStatus(id: string, status: string): Promise<{ error: string | null }> {
  try {
    const { error } = await supabase.from('early_pickup_requests').update({ status }).eq('id', id);
    if (error) throw error;
    return { error: null };
  } catch (err: any) {
    console.warn('updatePickupStatus failed:', err);
    return { error: err.message || 'Intermittent connectivity' };
  }
}

// Fetch all buses
export async function fetchBuses(): Promise<any[]> {
  const cacheKey = `buses_all`;
  try {
    const { data, error } = await supabase.from('buses').select('*').order('number');
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchBuses failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Fetch parent's student
export async function fetchParentStudent(parentUserId: string): Promise<StudentRow | null> {
  const cacheKey = `parent_student_${parentUserId}`;
  try {
    const { data, error } = await supabase
      .from('students')
      .select('*')
      .eq('parent_user_id', parentUserId)
      .maybeSingle();
    if (error) throw error;
    
    const result = (data as StudentRow) ?? null;
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchParentStudent failed, loading cache:', err);
    const cached = await cacheGet<StudentRow>(cacheKey);
    return cached;
  }
}

// Fetch all user profiles (admin)
export async function fetchUsersByRole(role: string): Promise<any[]> {
  const cacheKey = `users_by_role_${role}`;
  try {
    const { data, error } = await supabase
      .from('user_profiles')
      .select('*')
      .eq('role', role)
      .eq('is_active', true)
      .order('display_name');
    if (error) throw error;
    
    const result = data ?? [];
    await cacheSet(cacheKey, result);
    return result;
  } catch (err) {
    console.warn('fetchUsersByRole failed, loading cache:', err);
    const cached = await cacheGet<any[]>(cacheKey);
    return cached ?? [];
  }
}

// Push notification helpers
export async function savePushToken(userId: string, token: string): Promise<void> {
  await supabase.from('user_profiles').update({ push_token: token }).eq('id', userId).catch(() => {});
}

export async function sendPushNotification(payload: {
  title: string; body: string; target_role?: string; target_user_id?: string;
  data?: any; created_by?: string;
}): Promise<void> {
  await supabase.from('push_notifications').insert(payload).catch(() => {});
}
