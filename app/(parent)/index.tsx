...
export default function ParentFeed() {
  ...
  const [parentXp, setParentXp] = useState(650);
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      setParentXp(p => Math.min(p + 50, 1000)); // Gain XP for uploading!
    }
  };
    ...
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
        ...
        <ResponsiveContainer maxWidth={600}>
          ...
          <View style={styles.row}>
            ...
            <Pressable onPress={pickImage}>
              {profileImage ? (
                <Image source={{ uri: profileImage }} style={styles.profilePic} contentFit="cover" />
              ) : (
                <View style={[styles.profilePic, { backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' }]}>
      ...
      >
        ...
        <View style={{ marginBottom: Spacing.md }}>
          <GamifiedProgress currentLevel={3} xp={parentXp} targetXp={1000} title="Parent Engagement" />
        </View>
        <View style={styles.dashboardGrid}>
          <Pressable style={styles.dashboardTile} onPress={() => router.push('/(parent)/bus-tracking' as any)}>
            <View style={[styles.tileIconWrap, { backgroundColor: Colors.successBg }]}>
              <MaterialCommunityIcons name="bus-marker" size={28} color={Colors.success} />
            </View>
            <Text style={styles.tileLabel}>Bus Tracking</Text>
          </Pressable>
          <Pressable style={styles.dashboardTile} onPress={() => router.push('/(parent)/academic' as any)}>
          ...
          <Pressable style={styles.dashboardTile} onPress={() => router.push('/(parent)/attendance' as any)}>
          ...
          <Pressable style={styles.dashboardTile} onPress={() => router.push('/(parent)/timetable' as any)}>
          ...
          <Pressable style={styles.dashboardTile} onPress={() => router.push('/(parent)/assistant' as any)}>
          ...
          <Pressable style={styles.dashboardTile} onPress={() => router.push('/(parent)/profile' as any)}>
...
const styles = StyleSheet.create({
  ...
  profilePic: { width: 48, height: 48, borderRadius: 24, borderWidth: 2, borderColor: '#fff' },
