...
const ADMIN_SIDEBAR = [
  ...
  { icon: 'account-arrow-right' as const, label: 'Pickup', route: '/(admin)/pickup' },
  { icon: 'alert-circle' as const, label: 'Incidents', route: '/(admin)/incidents' },
  { icon: 'bullhorn' as const, label: 'Notices', route: '/(admin)/notices' },
  { icon: 'file-chart' as const, label: 'Reports', route: '/(admin)/reports' },
  { icon: 'star-circle' as const, label: 'Special Module', route: '/(admin)/special-module' },
  { icon: 'brain' as const, label: 'AI Chatbot', route: '/(admin)/ai' },
  { icon: 'cog' as const, label: 'Settings', route: '/(admin)/settings' },
];
...
export default function AdminOverview() {
  ...
  return (
    <ResponsiveContainer sidebarItems={ADMIN_SIDEBAR} activeRoute="/(admin)">
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      ...
      {activeAlerts.length > 0 && (
        <View style={{ zIndex: 100 }}>
          <LinearGradient colors={['#7F1D1D', '#DC2626']} style={styles.emergencyBanner}>
            ...
            <Pressable onPress={() => router.push('/(admin)/emergency' as any)} style={styles.emergencyBtn}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Card padded={false}>
          <NavRow icon="account-off" label="Absence Management" tint={Colors.danger} bg={Colors.dangerBg} onPress={() => router.push('/(admin)/absence-management' as any)} />
          <Divider />
          <NavRow icon="calendar-clock" label="Substitute Timetable" tint={Colors.info} bg={Colors.infoBg} onPress={() => router.push('/(admin)/substitute-timetable' as any)} />
          <Divider />
          <NavRow icon="account-multiple-plus" label="Teacher Registration" tint={Colors.primary} bg={Colors.surfaceTint} onPress={() => router.push('/(admin)/teacher-registration' as any)} />
          <Divider />
          <NavRow icon="school" label="School Registration" tint={Colors.saffron} bg="#FFE9DC" onPress={() => router.push('/(admin)/school-registration' as any)} />
          <Divider />
          <NavRow icon="alert-circle" label="Emergency Alerts" tint={Colors.danger} bg={Colors.dangerBg} onPress={() => router.push('/(admin)/emergency' as any)} badge={activeAlerts.length > 0 ? `${activeAlerts.length} LIVE` : undefined} />
          <Divider />
          <NavRow icon="file-chart" label="Reports & Analytics" tint={Colors.success} bg={Colors.successBg} onPress={() => router.push('/(admin)/reports' as any)} />
          <Divider />
          <NavRow icon="star-circle" label="Special Module" tint="#Eab308" bg="#Fefce8" onPress={() => router.push('/(admin)/special-module' as any)} />
          <Divider />
          <NavRow icon="brain" label="AI Insights & Chatbot" tint="#6E55C2" bg="#F0ECFD" onPress={() => router.push('/(admin)/ai' as any)} />
          <Divider />
          <NavRow icon="account-arrow-right" label="Early Pickup Requests" tint={Colors.warning} bg={Colors.warningBg} onPress={() => router.push('/(admin)/pickup' as any)} badge={stats.pendingPickups > 0 ? `${stats.pendingPickups}` : undefined} />
          <Divider />
          <NavRow icon="bus-multiple" label="Fleet Management" tint={Colors.saffron} bg="#FFE9DC" onPress={() => router.push('/(admin)/fleet' as any)} />
          <Divider />
          <NavRow icon="cog" label="Settings" tint={Colors.textMuted} bg="#F2F2F7" onPress={() => router.push('/(admin)/settings' as any)} />
        </Card>
...
function Op({ icon, label, value, tone, bg }: { icon: keyof typeof MaterialCommunityIcons.glyphMap; label: string; value: string; tone: string; bg: string }) {
  const layout = useResponsive();
  const cardW = layout.isDesktop ? '15%' : layout.isTablet ? '23%' : '31%';
  return (
    <View style={[styles.opCard, Shadows.card, { width: cardW }]}>
      <View style={[styles.opIcon, { backgroundColor: bg }]}>
        <MaterialCommunityIcons name={icon} color={tone} size={layout.isDesktop ? 26 : 22} />
      </View>
      <Text style={[styles.opValue, layout.isDesktop && { fontSize: 24 }]}>{value}</Text>
      <Text style={[styles.opLabel, layout.isDesktop && { fontSize: 12 }]}>{label}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  opCard: { backgroundColor: '#fff', borderRadius: Radius.lg, padding: Spacing.lg },
