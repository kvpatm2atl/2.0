...
export default function BusDriverIndex() {
  ...
  const updateStatus = async (busId: string, newStatus: string) => {
    ...
    // Add real location if on route
    if (newStatus === 'On Route' || newStatus === 'Returning') {
      const { status } = await Location.requestForegroundPermissionsAsync();
      if (status === 'granted') {
        try {
          const loc = await Location.getCurrentPositionAsync({});
          updates.latitude = loc.coords.latitude;
          updates.longitude = loc.coords.longitude;
        } catch (e) {
          console.log("Could not get location", e);
          // Fallback coords
          updates.latitude = 8.5241;
          updates.longitude = 76.9366;
        }
      }
    }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {/* Emergency & Tools */}
        <Text style={styles.sectionTitle}>Tools & Emergency</Text>
        <View style={{ flexDirection: 'row', gap: 12 }}>
          <Pressable onPress={() => router.push('/(bus_driver)/student-lookup' as any)} style={styles.toolBtn}>
            <MaterialCommunityIcons name="card-search" color={Colors.primary} size={24} />
            <Text style={styles.toolText}>Student Lookup</Text>
          </Pressable>
          <Pressable onPress={() => showAlert('Emergency Alert', 'SOS sent to admin and school management.', [{ text: 'OK' }])}
            style={[styles.toolBtn, { backgroundColor: Colors.danger }]}>
            <MaterialCommunityIcons name="alert-octagon" color="#fff" size={24} />
            <Text style={[styles.toolText, { color: '#fff' }]}>Send SOS</Text>
          </Pressable>
        </View>
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  toolText: { color: Colors.primary, fontSize: 14, fontWeight: '800' },
});
