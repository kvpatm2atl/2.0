...
import { ActivityIndicator, FlatList, KeyboardAvoidingView, Modal, Platform, Pressable, ScrollView, StyleSheet, Text, TextInput, View, Image } from 'react-native';
...
export default function SecurityPickup() {
  ...
  const takePhoto = async () => {
    ...
    if (status !== 'granted') {
      showAlert('Permission required', 'Need camera permission to take visitor photos.');
      return;
    }
  ...
  const submitGatePickup = async () => {
    ...
    if (!visitorPhoto) { showAlert('Missing photo', 'Please take a photo of the visitor.'); return; }
    setSaving(true);
    
    // Mock the pending state and notification
    addNotification({
      title: 'Action Required: Early Pickup Request',
      message: `${authorizedPerson} is at the gate requesting an early pickup for ${selectedStudent.name}. Please confirm.`,
    ...
    const { error } = await createPickupRequest({
      ...
      status: 'Pending', // Wait for parent approval
    });
    setSaving(false);
    if (error) { showAlert('Error', error); return; }
    showAlert('Request Sent', `Sent confirmation request to ${selectedStudent.name}'s parent. Please wait for approval.`);
    setShowForm(false);
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }} edges={['top', 'bottom']}>
            ...
            <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
              ...
              <Text style={[styles.label, { marginTop: Spacing.lg }]}>Visitor Photo</Text>
              <Pressable onPress={takePhoto} style={styles.photoUploadBox}>
                {visitorPhoto ? (
                  <Image source={{ uri: visitorPhoto }} style={styles.previewImage} />
                ) : (
                  <>
                    <MaterialCommunityIcons name="camera" color={Colors.textMuted} size={32} />
                    <Text style={styles.photoUploadText}>Tap to take picture</Text>
                  </>
                )}
              </Pressable>
...
const styles = StyleSheet.create({
  ...
  photoUploadBox: { marginTop: 8, backgroundColor: Colors.surfaceMuted, borderRadius: Radius.md, height: 150, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderStyle: 'dashed', borderColor: Colors.border, overflow: 'hidden' },
  photoUploadText: { color: Colors.textMuted, marginTop: 8, fontWeight: '600' },
